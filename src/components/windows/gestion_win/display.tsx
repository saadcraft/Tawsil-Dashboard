        ...
        <div className='fixed z-20 top-0 flex items-center bottom-0 right-0 left-0 md:left-80 p-5 bg-opacity-50 bg-slate-700'>
            <div className='xl:w-2/3 w-full mx-auto rounded-xl p-10 mt-10 bg-white'>
                ...
                <div className='sm:flex justify-around'>
                    <div>
                        <p><span className='font-bold'>Nom Et Prénome : </span> {`${document.user.last_name} ${document.user.first_name}`}</p>
                        <p><span className='font-bold'>Email : </span> {`${document.user.email}`}</p>
                        <p><span className='font-bold'>Télephoen : </span> {`${document.user.phone_number_1}`}</p>
                        <p><span className='font-bold'>Télephoen 2 : </span> {` ${document.user.phone_number_2 ? document.user.phone_number_2 : '/'}`}</p>
