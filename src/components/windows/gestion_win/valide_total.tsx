...
import PrintableModelCousesGroup from '@/lib/tools/printer_models/printer_grouped_courses';

export function ValideCommande({ command, onEvent, onBack }: { command: GroupeVTC, onEvent: () => void, onBack: () => void }) {
    return (
            ...
            <div className='relative overflow-auto  w-full h-full mx-auto rounded-xl p-5 bg-white'>
                ...
                <div className="flex justify-center items-center h-5/6 gap-10 rounded-lg">
                    ...
                    <div className="text-lg border-l pl-4 space-y-4 text-gray-600">
                        <h1>{command.partener__user__last_name || "/"}</h1>
                        <h1>{command.partener__user__first_name || "/"}</h1>
                        <h1>{command.partener__user__phone_number_1 || "/"}</h1>
...
export function ValideSecond({ command, onEvent, onBack }: { command: GroupeVTC, onEvent: () => void, onBack: () => void }) {
                        ...
                        <tbody>
                            ...
                            <tr className="bg-white border-b dark:border-gray-700">
                                ...
                                <td className="px-6 py-4">
                                    {command.partener__user__first_name} {command.partener__user__last_name}
                                </td>
                                <td className="px-6 py-4 text-right">
                                    {command.total_tax_wi}DA
                                </td>
                            </tr>

                        </tbody>
                    </table>
                    <h1 className='text-right p-2 font-semibold text-xl'>total {command.total_tax_wi.toFixed(2)} DA</h1>
                </div>
                <div className='absolute bottom-3 right-3 flex gap-4 text-xl justify-end'>
                    <button onClick={onBack} className='disabled:text-slate-400'>Retour</button>
                    <button onClick={onEvent} className=''>Suivant</button>
...
export function ValideThird({ command, onBack, onSub, user }: { command: GroupeVTC, onBack: () => void, onSub: (data: number) => void, user: Users }) {
        ...
        <div className='fixed z-20 overflow-auto top-20 flex items-start bottom-0 right-0 left-0 md:left-80 p-5 bg-opacity-50 bg-slate-700'>
            ...
            <div style={{ display: 'none' }}>
                <div ref={componentRef}>
                    <PrintableModelCousesGroup command={command} user={user} />
                </div>
            </div>
        </div>
    )
