    ...
    return (
        <div className='fixed z-10 overflow-auto top-20 flex items-center bottom-0 right-0 left-0 md:left-80 p-5 bg-opacity-50 bg-slate-700'>
            <div className='xl:w-1/3 w-full mx-auto p-5 mt-10 bg-white'>
                <h1 className='mb-5 text-2xl font-bold text-center'>Ajouter type de compte</h1>
                <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
                    ...
                    <p>Tax</p>
                    <input type='number' name='tax' className='p-2 border border-slate-300 rounded-md' placeholder='Entre le Taxe' />
                    <button className='bg-green-600 disabled:bg-opacity-20 px-4 py-2 text-white rounded-lg font-semibold'>Créer</button>
                </form>
            </div>
        </div>
    )
