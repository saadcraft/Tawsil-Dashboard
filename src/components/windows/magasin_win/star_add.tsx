                ...
                <form onSubmit={handleCreate} className='flex flex-col gap-4' encType="multipart/form-data">
                    ...
                    <div className='flex justify-between'>
                        ...
                        <div className='flex gap-2'>
                            <span onClick={() => setVariats(prev => (prev > 1 ? prev - 1 : prev))}>-</span>
                            <span onClick={() => setVariats(prev => prev + 1)}>+</span>
                        </div>
                    </div>
                    {Array.from({ length: variants }).map((val, index) => (
                        ...
                        <div key={index} className='flex w-full'>
                            <input type='text' name='price' className='p-2 border border-slate-300 rounded-md' placeholder='Entre le prix' />
                            <input type='text' name='quantity' className='p-2 border border-slate-300 rounded-md' placeholder='Entre le quantity' />
                        </div>
                    ))
                    }
                    <p>description</p>
