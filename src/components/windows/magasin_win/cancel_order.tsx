...
export default function CancelCommande({ id, onsub }: { id: number, onsub: (value: null) => void }) {

    const router = useRouter()

    const handleDelete = async (commande_id: number) => {

        const res = await changeStatus({ commande_id, status: "canceled" })
        if (res) {
            router.refresh()
            onsub(null)
        }
    ...
    return (
        <div className='fixed z-20 top-0 flex items-center bottom-0 right-0 left-0 md:left-80 p-5 bg-opacity-50 bg-slate-700'>
            <div className='max-w-3xl rounded-xl mx-auto p-5 mt-10 bg-white'>
                <h1 className='mb-5 font-bold text-center text-3xl'>est ce que vous êtes sur de annuler ce commande</h1>
                <div className='flex gap-3 justify-center'>
                    <button onClick={() => handleDelete(id)} className='bg-green-600 disabled:bg-opacity-20 px-4 py-2 text-white rounded-lg font-semibold'>Yes</button>
